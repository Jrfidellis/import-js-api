...
import type JsModule from './JsModule';
...
export function runCommand(
  ...
  return initializeModuleFinder(workingDirectory)
    .then((): Promise<*> => getLines(pathToFile).then((lines: Array<string>): Promise<*> => {
      const importer = new Importer(lines, pathToFile, workingDirectory);
      return executor(importer).then((result: Object) => {
        if (overwrite) {
          fs.writeFile(pathToFile, result.fileContent, (err: ?ErrnoError) => {
            if (err) throw err;
            process.exit(0);
          });
        } else {
...
program
  ...
  .option(...sharedOptions.overwrite)
  .action((word: string, pathToFile: string, options: Object) => {
    const executor = (importer: Importer): Promise<Array<JsModule>> =>
      importer.search(word).then(({
        modules,
      }: {
        modules: Array<JsModule>,
      }): Array<JsModule> => modules);
    runCommand(executor, pathToFile, options);
  });
...
program
  ...
  .action((imports: string, pathToFile: string, options: Object) => {
    const executor = (importer: Importer): Promise<Object> =>
      importer.addImports(JSON.parse(imports));
    runCommand(executor, pathToFile, options);
  });

program.command('goto <word> <pathToFile>').action((
  word: string,
  pathToFile: string,
) => {
  const workingDirectory = findProjectRoot(pathToFile);
  initializeModuleFinder(workingDirectory)
    .then((): Promise<*> => getLines(pathToFile).then((lines: Array<string>) => {
...
export function importjs(argv: Array<string>) {
  program.parse(argv);
